...
class Serializer {
  ...
  markRecursive(object) {
    ...
    // is the given object parameter a class that is in the types array? thus a class to serialize.
    let idx = this.types.findIndex((t) => {
      return t.name === object.constructor.name;
    });

    if (idx !== -1) {
      // tag the object with the index of the type in the types array given as a parameter to the constructor
      object["typeIndex"] = idx;

      for (let key in object) {
        if (object.hasOwnProperty(key) && object[key] != null) console.log(key);
  ...
  reconstructRecursive(object) {
    // is the object a class that need to have deep copy?
